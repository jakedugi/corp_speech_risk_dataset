...
Test integration of complex extraction patterns into the main case outcome imputer.
"""
...
class ComplexIntegrationTester:
    """Test complex extraction integration with missing cases."""
    ...
    def test_complex_extraction_on_missing_cases(self):
        """Test complex extraction on the 9 missing cases."""
        df = self._load_gold_standard()

        print("🧪 Testing Complex Extraction on Missing Cases")
        print("=" * 80)
        ...
        # Test complex extraction on each missing case
        for case in missing_cases:
            ...
            # Test complex extraction on the actual context
            complex_candidates = extract_complex_amount_candidates(
                case["actual_context"], min_amount=100
            )

            if complex_candidates:
                print(
                    f"✅ Complex extraction found {len(complex_candidates)} candidates:"
                )
                for i, candidate in enumerate(complex_candidates, 1):
                    print(f"   {i}. ${candidate['value']:,.0f} ({candidate['type']})")
                    print(f"      Raw: '{candidate['amount']}'")
                    print(f"      Context: ...{candidate['context'][:100]}...")

                    # Check if this matches the actual amount
                    if abs(candidate["value"] - case["actual_amount"]) <= 1.0:
                        print(f"      🎯 EXACT MATCH FOUND!")
                    elif (
                        abs(candidate["value"] - case["actual_amount"])
                        / case["actual_amount"]
                        <= 0.1
                    ):
                        print(f"      🎯 CLOSE MATCH (within 10%)")
            else:
                print("❌ Complex extraction found no candidates")
...
def main():
    """Run complex extraction integration test."""
    tester = ComplexIntegrationTester()

    # Run test
    missing_cases = tester.test_complex_extraction_on_missing_cases()

    print(f"\n🎉 Complex extraction integration test completed!")
