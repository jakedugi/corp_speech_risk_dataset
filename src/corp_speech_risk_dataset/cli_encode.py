...
@click.option(
    ...
    help="Root under which to look for *_stage4.jsonl files.",
)
@click.option(
    "--tokenized-root",
    type=click.Path(file_okay=False, dir_okay=True, path_type=Path),
    default=Path("data/tokenized"),
    help="Root where tokenized JSONL will be written.",
)
@click.option(
    "-r",
    "--recursive",
    is_flag=True,
    help="If IN_PATH is a directory, walk it recursively for *_stage4.jsonl files.",
)
def main(
    in_path: Path,
    recursive: bool,
    ...
    tokenized_root = tokenized_root.resolve()
