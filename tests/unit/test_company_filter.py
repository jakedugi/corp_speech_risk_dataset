...
from corp_speech_risk_dataset.api.courtlistener import process_statutes, STATUTE_QUERIES
...
def create_test_company_file():
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as f:
        ...
        writer.writerow(['AAPL', 'Apple Inc.', 'https://example.com/apple', '0000320193'])
        writer.writerow(['MSFT', 'Microsoft Corporation', 'https://example.com/microsoft', '0000789019'])
        writer.writerow(['GOOGL', 'Alphabet Inc.', 'https://example.com/alphabet', '0001652044'])
        return f.name
    ...
    try:
        ...
        try:
            ...
            if "API token" in str(e) or "401" in str(e) or "403" in str(e):
                print("✓ process_statutes accepts company_file parameter (failed as expected due to API token)")
            else:
