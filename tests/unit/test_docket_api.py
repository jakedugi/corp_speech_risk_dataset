...
from corp_speech_risk_dataset.api.courtlistener import CourtListenerClient, process_statutes, process_recap_data, process_docket_entries, process_recap_documents, process_full_docket
...
def main():
    ...
    try:
        ...
        if not config.api_token:
            print("Warning: No API token found. Set COURTLISTENER_API_TOKEN environment variable.")
            return
    ...
    try:
        ...
        recap_data = client.fetch_recap_data(
            query="FTC",
            pages=1,
            page_size=5
        )
    ...
    try:
        ...
        if opinions:
            ...
            if first_opinion.get('cluster'):
                ...
                print("✓ Full docket methods available (requires specific docket ID for full test)")
            else:
